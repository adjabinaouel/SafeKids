// src/components/Navigation/DoctorNavigation.js
...
const TABS = [
  ...
  { key: 'addChild',  icon: 'user-plus',  label: 'Ajouter Enfant' },
  { key: 'children',  icon: 'users',      label: 'Mes Patients' },
  { key: 'notifications', icon: 'bell',   label: 'Notifs'    },
  { key: 'profile',   icon: 'user',       label: 'Profil'    },
];

const SCREEN_MAP = {
  dashboard:     'DashboardMedecin',
  addChild:      'AddChildren',
  children:      'Patients',
  notifications: 'NotificationsMedecin',
  profile:       'ProfilMedecin',
};
...
const DoctorLayout = ({ children, activeTab }) => {
  ...
  const navigateTo = (key) => {
    const screenName = SCREEN_MAP[key];
    if (screenName) {
      ...
      <View style={{ flex: 1, flexDirection: 'row', backgroundColor: COLORS.white }}>
        ...
        <View style={{
          ...
          <View style={{
            ...
            <Text style={{ fontSize: 10, color: COLORS.textMuted, fontWeight: '500' }}>SafeKids Médecin v2.1.0</Text>
