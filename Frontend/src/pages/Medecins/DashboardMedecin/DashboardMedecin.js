// src/pages/Medecins/DashboardMedecin/DashboardMedecin.js
...
import S from './DashboardMedecinStyles';
...
const GUIDE_STEPS = [
  {
    ...
    desc: "Commencez par ajouter les informations des enfants à suivre. Remplissez le formulaire avec les détails médicaux.",
    action: 'AddChildren', color: '#8B5CF6', bg: '#EDE9FE',
  },
  {
    step: '02', icon: 'users',
    title: 'Gérer les patients',
    desc: "Consultez la liste de vos patients, mettez à jour leurs informations et suivez leur évolution.",
    action: 'ChildrenList', color: '#0EA5E9', bg: '#E0F2FE',
  },
  {
    step: '03', icon: 'activity',
    title: 'Suivre les progrès',
    desc: "Utilisez les activités thérapeutiques pour suivre les progrès de chaque enfant.",
    action: 'Activities', color: '#10B981', bg: '#D1FAE5',
  },
];
...
const SLabel = ({ tag, title, sub }) => (
  <View style={{ marginBottom: 18 }}>
    ...
    {sub ? <Text style={{ fontSize: 13, color: '#94A3B8', lineHeight: 18 }}>{sub}</Text> : null}
  </View>
);
...
export default function DashboardMedecin() {
  ...
  const goToProfile = () => {
    const routes = ['DoctorProfile', 'Profile', 'MonProfil'];
      ...
      <View style={{ flex: 1, backgroundColor: '#F8F7FF' }}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}
          ...
          >
            ...
            <Animated.View style={{
              ...
              <View style={{ gap: 10, flexDirection: 'row' }}>
                {/* Patients button */}
                <TouchableOpacity onPress={() => navigation.navigate('Patients')} activeOpacity={0.75} style={{
                  width: 46, height: 46, borderRadius: 15,
                  backgroundColor: 'rgba(167,139,250,0.22)',
                  borderWidth: 1.5, borderColor: 'rgba(167,139,250,0.45)',
                  justifyContent: 'center', alignItems: 'center',
                }}>
                  <Feather name="users" size={20} color="#A78BFA" />
                </TouchableOpacity>
                {/* Notifications button */}
                <TouchableOpacity onPress={() => navigation.navigate('NotificationsMedecin')} activeOpacity={0.75} style={{
                  width: 46, height: 46, borderRadius: 15,
                  backgroundColor: 'rgba(252,211,77,0.22)',
                  borderWidth: 1.5, borderColor: 'rgba(252,211,77,0.45)',
                  justifyContent: 'center', alignItems: 'center',
                }}>
                  <Feather name="bell" size={20} color="#FCD34D" />
                </TouchableOpacity>
                {/* Profile button */}
                <TouchableOpacity onPress={goToProfile} activeOpacity={0.75} style={{
                  width: 46, height: 46, borderRadius: 15,
                  backgroundColor: 'rgba(252,211,77,0.22)',
                  borderWidth: 1.5, borderColor: 'rgba(252,211,77,0.45)',
                  justifyContent: 'center', alignItems: 'center',
            ...
            <Animated.View style={{
              ...
              <View style={{ flexDirection: 'row', gap: 12, marginBottom: 12 }}>
                <StatCard icon="users"       title="Patients"     value="—" sub="En suivi"      color="#C4B5FD" delay={80}  />
                <StatCard icon="activity"    title="Consultations" value="—" sub="Ce mois"      color="#6EE7B7" delay={160} />
              </View>
              <View style={{ flexDirection: 'row', gap: 12 }}>
                <StatCard icon="user-plus"   title="Nouveaux"      value="—" sub="Cette semaine" color="#FCD34D" delay={240} />
                <StatCard icon="trending-up" title="Progrès"       value="—" sub="Moyen"        color="#93C5FD" delay={320} />
              </View>
            </Animated.View>
          ...
          <Animated.View style={{
            ...
            <SLabel tag="ACTIONS PRINCIPALES" title="Gestion des patients" sub="Accédez rapidement aux fonctionnalités essentielles" />
            <View style={{ flexDirection: 'row', gap: 12, marginBottom: 12 }}>
              <NavCard icon="user-plus" title="Ajouter Enfant" subtitle="Nouveau patient" color="#7C3AED" bg="#EDE9FE" delay={80}  onPress={() => navigation.navigate('AddChildren')} />
              <NavCard icon="users"     title="Mes Patients"   subtitle="Liste complète"  color="#0EA5E9" bg="#E0F2FE" delay={160} onPress={() => navigation.navigate('Patients')} />
            </View>
            <View style={{ flexDirection: 'row', gap: 12 }}>
              <NavCard icon="activity"  title="Activités"      subtitle="Thérapeutiques"  color="#10B981" bg="#D1FAE5" delay={240} onPress={() => navigation.navigate('Activities')} />
              <NavCard icon="bar-chart-2" title="Statistiques" subtitle="Suivi médical"  color="#F59E0B" bg="#FEF3C7" delay={320} onPress={() => navigation.navigate('DoctorStats')} />
            </View>
          </Animated.View>
          ...
          <Animated.View style={{
            ...
            <SLabel tag="GUIDE D'UTILISATION" title="Comment démarrer ?" sub="Suivez ces étapes pour bien organiser votre pratique médicale." />
          ...
          <View style={{ alignItems: 'center', paddingHorizontal: 22, paddingTop: 4, paddingBottom: 12 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#fff', borderRadius: 20, paddingHorizontal: 18, paddingVertical: 10, borderWidth: 1, borderColor: 'rgba(139,92,246,0.10)' }}>
              ...
              <Text style={{ fontSize: 11, color: '#94A3B8', fontWeight: '600' }}>SafeKids Médecin · v2.1.0 · © 2026</Text>
            </View>
          </View>
