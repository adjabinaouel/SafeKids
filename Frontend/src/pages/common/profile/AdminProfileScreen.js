...
const PermissionBadge = ({ label, granted }) => (
  <View style={{
    ...
    <View style={{
      ...
      backgroundColor: granted ? '#D1FAE5' : '#FEE2E2',
...
export default function AdminProfileScreen({ navigation }) {
  ...
  // ✅ avatarUri séparé — jamais écrasé par un reload
  ...
  useEffect(() => {
    const init = async () => {
      // ✅ Charger l'avatar mis en cache localement d'abord
  ...
  const loadAdminProfile = async () => {
    try {
      ...
      // ✅ Avatar : priorité au cache local, sinon URL serveur
      const cached = await AsyncStorage.getItem('adminCachedAvatarUri');
      if (!cached && data.avatar) {
        const serverAvatar = `${SERVER_URL}${data.avatar}?t=${Date.now()}`;
        setAvatarUri(serverAvatar);
        await AsyncStorage.setItem('adminCachedAvatarUri', serverAvatar);
      }
  ...
  try {
    ...
    // Lecture en Base64 pur (comme tu fais déjà)
    ...
    await AsyncStorage.setItem('cachedAvatarUri', serverUri); // ou admin...
  ...
  return (
    <AdminLayout activeTab="profile">
      <View style={{ flex: 1, backgroundColor: '#F8FAFC' }}>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)', marginTop: 2 }}>
              Accès privilégié · {userData.niveau}
            </Text>
          </View>
          {isEditing && (
            <TouchableOpacity
        ...
        {/* ── SCROLL ──────────────────────────────────────────────────────── */}
        {/* ✅ paddingBottom 110 pour éviter que les boutons soient sous la navbar */}
        <Animated.ScrollView
        ...
        >
          ...
          {/* ── JOURNAL ──────────────────────────────────────────────────── */}
          <View style={{ paddingHorizontal: 16, marginBottom: 12 }}>
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#1E293B', marginBottom: 10 }}>
            ...
            <TouchableOpacity activeOpacity={1}>
              <View style={{
                  ...
                  <View key={label} style={{ marginBottom: 14 }}>
                    ...
                    <TextInput
                      style={{
                        ...
                        height: 50, fontSize: 15, color: '#1E293B',
                ...
                <TouchableOpacity
                  ...
                  style={{
                    height: 52, borderRadius: 16, backgroundColor: '#4C1D95',
                    justifyContent: 'center', alignItems: 'center',
